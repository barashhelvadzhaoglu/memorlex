...
export default async function SubjectSelectionPage({ params }: { params: Promise<{ subject: string }> }) {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-6 bg-slate-50 dark:bg-[#0f172a] transition-colors duration-300">
      {/* Başlık: Koyu modda beyaza döner */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-2xl">
        ...
        >
          ...
          <h2 className="font-black text-xl text-slate-800 dark:text-slate-100 mb-1">Kelime Listesi</h2>
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Öğrenmeye başla</p>
        </Link>

        {/* Flashcards Kartı */}
        <Link 
          href={`/tr/${subject}/flashcards`} 
        ...
        >
          ...
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Kendini test et</p>
        </Link>
      </div>
    </main>
  );
